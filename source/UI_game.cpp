...
void CUI_game::GameUpdate(void)
{
	...
	if (CRenderer::GetMode() == CRenderer::MODE_GAME)
	{
		...
		// ゲーム背景
		SetUI(D3DXVECTOR3(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, 0.0f), SCREEN_WIDTH, SCREEN_HEIGHT, LOGOTYPE_GAMEBG, WhiteColor);

		if (!m_bTransform[0])
		{
			// 宝石背景1P
			SetUI(D3DXVECTOR3(200, STONE_POS_Y, 0), 210, STONE_SIZE_Y, LOGOTYPE_JEWELRYBG1P, WhiteColor);

			// 1Pが赤石を取ったとき
			if (m_bStoneID[0][0])
			{
				// 宝石赤
				SetUI(D3DXVECTOR3(125, STONE_POS_Y, 0), STONE_SIZE_X, STONE_SIZE_Y, LOGOTYPE_JEWELRYRED, WhiteColor);
			}
			...
			{
				// 宝石青
				SetUI(D3DXVECTOR3(200, STONE_POS_Y, 0), STONE_SIZE_X, STONE_SIZE_Y, LOGOTYPE_JEWELRYGREEN, WhiteColor);
			}
			...
			{
				// 宝石緑
				SetUI(D3DXVECTOR3(275, STONE_POS_Y, 0), STONE_SIZE_X, STONE_SIZE_Y, LOGOTYPE_JEWELRYBULE, WhiteColor);
			}
		}
		else
		{
		...
		{
			// 宝石背景2P
			SetUI(D3DXVECTOR3(1080, STONE_POS_Y, 0), 210, STONE_SIZE_Y, LOGOTYPE_JEWELRYBG2P, WhiteColor);

			// 2Pが赤石を取ったとき
			if (m_bStoneID[1][0])
			{
				// 宝石赤
				SetUI(D3DXVECTOR3(1005, STONE_POS_Y, 0), STONE_SIZE_X, STONE_SIZE_Y, LOGOTYPE_JEWELRYRED, WhiteColor);
			}
			...
			{
				// 宝石青
				SetUI(D3DXVECTOR3(1080, STONE_POS_Y, 0), STONE_SIZE_X, STONE_SIZE_Y, LOGOTYPE_JEWELRYGREEN, WhiteColor);
			}
			...
			{
				// 宝石緑
				SetUI(D3DXVECTOR3(1155, STONE_POS_Y, 0), STONE_SIZE_X, STONE_SIZE_Y, LOGOTYPE_JEWELRYBULE, WhiteColor);
			}
		}
		else
		{
		...
		// 1PキャラクターUI
		SetUI(D3DXVECTOR3(55, 70, 0), 110, 130, LOGOTYPE_PLAYER1, WhiteColor);
		// テクスチャ設定
		m_pScene2D[LOGOTYPE_PLAYER1]->SetAnimation(0.25f, 1.0f, 0.0f, m_nCharaNum[0] / 2);
		// 2PキャラクターUI
		SetUI(D3DXVECTOR3(1225, 70, 0), 110, 130, LOGOTYPE_PLAYER2, WhiteColor);
		// テクスチャ設定
		m_pScene2D[LOGOTYPE_PLAYER2]->SetAnimation(0.25f, 1.0f, 0.0f, m_nCharaNum[1] / 2);

		// 1PキャラクターネームUI
		SetUI(D3DXVECTOR3(205, 155, 0), 400, 50, LOGOTYPE_CHARANAME0, WhiteColor);
		// テクスチャ設定
		m_pScene2D[LOGOTYPE_CHARANAME0]->SetAnimation(1.0f, 0.25f + (m_nCharaNum[0] / 2) * 0.25f, 0.0f + (m_nCharaNum[0] / 2) * 0.25f, 0);
		// 2PキャラクターネームUI
		SetUI(D3DXVECTOR3(1075, 155, 0), 400, 50, LOGOTYPE_CHARANAME1, WhiteColor);
		// テクスチャ設定
...
// ロゴ作成
//==================================================================================================================
void CUI_game::SetUI(D3DXVECTOR3 pos, float fSizeX, float fSizeY, int nCnt, D3DXCOLOR col)
{
